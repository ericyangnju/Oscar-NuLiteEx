...
{
	// Start NuLiteEx decode lib to decode NuLiteEx file stored from current audio id and played from audio channel 0.
	// The ROM file is placed on SPI Flash address "AUDIOROM_STORAGE_START_ADDR".
	// And decode the first frame of PCMs.
	if ( NuLiteExApp_DecodeStartPlayByID(&g_sApp.sNuLiteExAppDecode, g_sApp.u32PlayID, AUDIOROM_STORAGE_START_ADDR, 0) == FALSE )
		return FALSE;
